...
{
    SDL_FRect futurePos = player->hitBox;
    futurePos.x = futureX;
    futurePos.y = futureY;
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        if (&players[i] == player)
            continue;
        if (collision(futurePos, players[i].hitBox))
            return true;
    }
    ...
    if (tileCollision(world, futurePos, player, NULL))
        return true;

    return false;
}

void syncPlayerHitbox(Player *player)
{
    float hbW = player->hitBox.w;
    float hbH = player->hitBox.h;

    float offsetX = (PLAYER_SIZE * RENDER_SCALE - hbW) / 2.0f;
    float offsetY = (PLAYER_SIZE * RENDER_SCALE - hbH) - (4 * RENDER_SCALE);

    player->hitBox.x = player->pos.x + offsetX;
    player->hitBox.y = player->pos.y + offsetY;
}
...
void movement(Player *player, Player players[MAX_PLAYERS], Enemy enemies[MAX_ENEMIES], int deltatime, World world)
{
    ...
    {
        if (!willCollide(player, players, enemies, player->pos.x - player->flags.moveX * deltatime * SPEED, player->pos.y, world))
        {
    ...
    {
        if (!willCollide(player, players, enemies, player->pos.x, player->pos.y - player->flags.moveY * deltatime * SPEED, world))
        {
            player->pos.y -= player->flags.moveY * deltatime * SPEED;
        }
        player->facing = player->flags.moveY + 2;
    }
    syncPlayerHitbox(player);
}
...
bool playerEnemyCollision(Player *player, Enemy enemies[MAX_ENEMIES], Uint32 deltatime)
{
    ...
    for (int i = 0; i < MAX_ENEMIES; i++)
{
    if (enemies[i].texture == NULL || enemies[i].state == ENEMY_DEAD) continue;

    SDL_FRect dotDmgHitBox = enemies[i].hitBox;

    float padding = RENDER_SCALE * 6; 

    dotDmgHitBox.x -= padding;
    dotDmgHitBox.y -= padding;
    dotDmgHitBox.w += padding * 2;
    dotDmgHitBox.h += padding * 2;
...
void animatePlayers(Player players[MAX_PLAYERS], Uint8 *counter, Uint16 framerate, bool *flag)
{
    ...
    if ((*counter) == (framerate / ANIMATION_TIME / 2) || (*counter) == (framerate / (ANIMATION_TIME)))
    {
        for (int i = 0; i < MAX_PLAYERS; i++)
        {
            if (players[i].flags.moveX || players[i].flags.moveY)
            { // Render faster if moving
...
void updateClass(Player *player, SDL_Renderer *renderer)
{
    SDL_Surface *pArt;
    switch (player->class)
    { // Paths below should be change in the future
    case CLASS_NONE:
...
{
    float hbW = 12 * RENDER_SCALE;
    float hbH = 5 * RENDER_SCALE;
    player->hitBox.w = hbW;
    player->hitBox.h = hbH;
    float offsetX = (PLAYER_SIZE * RENDER_SCALE - hbW) / 2.0f;
    float offsetY = (PLAYER_SIZE * RENDER_SCALE - hbH) - (4 * RENDER_SCALE);
